...
# Canonical engine: turkey uses the selected cut's yield and purchase unit.
...
# Presentation passes the selected turkey metadata to MeatEngine.
...
# Replace duplicate side engine tests with tests against the actual production side block.
...
test('production side catalog is complete and alphabetized',()=>{for(const id of restored){assert.ok(sides[id]);assert.ok(sideOrder.includes(id));}assert.deepEqual(sideOrder,expected);});
...
# Add canonical turkey regression coverage.
p=Path('test/meat-engine.test.js'); s=p.read_text()
if 'turkey uses the selected cut' not in s:
    p.write_text(s+'''\n\ntest('turkey uses the selected cut yield and purchase unit',()=>{const whole=row('turkey',48,standard,{yield:.55,unitWeight:14});const breast=row('turkey',48,standard,{yield:.65,unitWeight:7});const legs=row('turkey',48,standard,{yield:.45,unitWeight:.75});close(whole.raw,16/.55);assert.equal(whole.units,3);assert.equal(whole.buyWeight,42);close(breast.raw,16/.65);assert.equal(breast.units,4);assert.equal(breast.buyWeight,28);close(legs.raw,16/.45);assert.equal(legs.units,48);assert.equal(legs.buyWeight,36);});\n''')
