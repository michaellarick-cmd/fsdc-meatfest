...
function multiplier(n,serv){const t={".5":[1,.6,.376455,.2857,.25,.22221,.19442,.1782092],".333333":[1,.4,.25097,.1904667,.1666667,.14814,.1296133,.1188061],".25":[.75,.3333,.2,.1667,.125,.111105,.0909,.0817429]};const v=Number(serv);const key=Math.abs(v-.5)<1e-6?".5":Math.abs(v-.25)<1e-6?".25":".333333";return (t[key])[Math.max(1,Math.min(8,n))-1]}
...
function familyPurchase(k,o,bought){
  ...
  if(k==="ribs"){
    ...
    const racks=halves/2, w=racks*2.25;
    let buy;
...
function calc(){let [adults,kids]=activeTotals(), eaters=adults+kids*.5, n=selected.size, mult=multiplier(n,+$("serving").value), rows=[],raw=0;
selected.forEach(k=>{
 let m=meats[k],o=m.options[choices[k]],per;
 if(k==="chicken")per=((.5+1/3)/2)*mult;else if(k==="fish")per=.25*mult;else if(k==="pork"||k==="hog")per=(1/3)*mult;else if(k==="brats")per=.25*mult;else per=.5*mult;
 let finished=eaters*per, targetFinished=planningMode==="family"?finished*1.125:finished, hogPlan=o.yield==="hog"?wholeHogPlan(targetFinished):null, y=hogPlan?hogPlan.yield:o.yield, bought=hogPlan?hogPlan.hangingWeight:targetFinished/y;
 let units=null,buyWeight=bought,purchaseNote="",excess=0,buyOverride=null;
 if(planningMode==="family" && k!=="hog"){
   const fp=familyPurchase(k,o,bought); if(fp){units=fp.units;buyWeight=fp.buyWeight;buyOverride=fp.buy;purchaseNote=fp.note;excess=Math.max(0,buyWeight-bought)}
 } else if(o.mode==="units"){
   units=Math.max(1,Math.ceil(bought/o.unitWeight));buyWeight=units*o.unitWeight;
   if(k==="brisket"&&choices[k]==="packer"){units=Math.max(1,Math.ceil(bought/14));buyWeight=units*14;excess=Math.max(0,buyWeight-bought);purchaseNote=cutNote(k,o)}
   else {excess=Math.max(0,buyWeight-bought);purchaseNote=cutNote(k,o);if(!purchaseNote&&excess>.5)purchaseNote=`Purchase amount is rounded to practical units; excess is planned leftovers.`}
...
$("results").innerHTML=rows.length?rows.map(r=>{let specialBuy=purchaseDisplay(r.k,r.o,r.units,r.buyWeight);let buy=r.buyOverride|| (r.o.mode==="weight"?`BUY ${Math.ceil(r.bought*10)/10} lb of fillets`:r.k==="hog"?`BUY 1 whole hog (~${Math.ceil(r.buyWeight)} lb hanging weight)`:r.k==="brisket"&&choices[r.k]==="packer"?`BUY ${r.units} ${r.o.unit}${r.units>1?"s":""} (~${r.units*14}–${r.units*18} lb total)`:specialBuy||`BUY ${r.units} ${r.o.unit}${r.units>1?"s":""} (~${Math.ceil(r.buyWeight*10)/10} lb total)`);let ex=r.excess>.5?` • Planned excess: <span class="excess">${Math.round(r.excess*10)/10} lb</span>`:"";let cushion=planningMode==="family"?` • Family cushion target: <b>${Math.round((r.targetFinished-r.finished)*10)/10} lb finished</b>`:"";return `<div class="result"><div class="resultTop"><div><div class="resultTitle">${r.m.name}</div><span class="pill">${r.o.label}</span><span class="pill">${Math.round(r.y*100)}% yield</span></div><div class="buy">${buy}</div></div><div class="details">Finished meat needed: <b>${Math.round(r.finished*10)/10} lb</b>${cushion} • Raw requirement: <b>${Math.round(r.bought*10)/10} lb</b>${r.o.mode==="units"&&r.k!=="hog"&&planningMode!=="family"?` • Planning unit: ${r.o.unitWeight} lb`:""}${ex}</div>${r.purchaseNote?`<div class="purchaseNote">${r.purchaseNote}</div>`:""}${r.k==="hog"?wholeHogWarning(r.buyWeight):""}</div>`}).join(""):"<p class='note'>Select at least one protein.</p>";calcSides();}
...
function buildSummary(){
 const [adults,kids]=activeTotals(),eaters=adults+kids*.5,rows=[];
 selected.forEach(k=>{const m=meats[k],o=m.options[choices[k]];let per,mult=multiplier(selected.size,+$("serving").value);
 if(k==="chicken")per=((.5+1/3)/2)*mult;else if(k==="fish")per=.25*mult;else if(k==="pork"||k==="hog")per=(1/3)*mult;else if(k==="brats")per=.25*mult;else per=.5*mult;
 const finished=eaters*per,targetFinished=planningMode==="family"?finished*1.125:finished,hogPlan=o.yield==="hog"?wholeHogPlan(targetFinished):null,y=hogPlan?hogPlan.yield:o.yield,bought=hogPlan?hogPlan.hangingWeight:targetFinished/y;let units=null,buyWeight=bought,buy,purchaseNote="";
 if(planningMode==="family"&&k!=="hog"){const fp=familyPurchase(k,o,bought);units=fp.units;buyWeight=fp.buyWeight;buy=fp.buy;purchaseNote=fp.note}
 else if(o.mode==="units"){units=Math.max(1,Math.ceil(bought/o.unitWeight));buyWeight=units*o.unitWeight;if(k==="brisket"&&choices[k]==="packer"){units=Math.max(1,Math.ceil(bought/14));buyWeight=units*14;buy=`${units} ${o.unit}${units>1?"s":""} (~${units*14}–${units*18} lb total)`;purchaseNote="Excess is planned leftovers."}else buy=purchaseDisplay(k,o,units,buyWeight)||`${units} ${o.unit}${units>1?"s":""} (~${Math.ceil(buyWeight*10)/10} lb total)`}
 else if(k==="hog")buy=`1 whole hog (~${Math.ceil(buyWeight)} lb hanging weight)`;else if(k==="fish")buy=`${units} filet${units===1?"":"s"} (~${Math.ceil(buyWeight*100)/100} lb total)`;else buy=`${Math.ceil(bought*10)/10} lb of fillets`;
 rows.push({k,m,o,finished,targetFinished,y,bought,buyWeight,buy,purchaseNote})});
