...
function loadEventDetails(){try{const x=JSON.parse(localStorage.getItem("mfEvent16")||"{}");const savedName=(x.name||"").trim();$("eventName").value=savedName&&savedName!=="Your Event"?savedName:"";$("eventDate").value=x.date||localISODate()}catch(e){$("eventName").value="";$("eventDate").value=localISODate()}if(!localStorage.getItem("mf11"))$("serving").value=".333333"}
...
$("file").onchange=e=>{const f=e.target.files[0];if(!f)return;const rd=new FileReader();rd.onload=()=>importText(rd.result);rd.readAsText(rd.result)};
