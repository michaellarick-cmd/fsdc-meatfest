// Family mode correction v1.3 — selected portion is the complete finished-meat target.
(function(){
  ...
  window.calc=function(){
    ...
    selected.forEach(k=>{
      ...
      let per=k==="chicken"?((.5+1/3)/2)*mult:k==="fish"?.25*mult:(k==="pork"||k==="hog")?(1/3)*mult:k==="brats"?.25*mult:k==="porkbelly"?(1/3)*.75*mult:.5*mult;
      ...
      if(k==="porkbelly"){
        buyWeight=bought;
        purchaseNote="Whole skinless bellies commonly run about 8–10 lb. Buy one whole belly, use approximately the calculated amount for this cook, and portion/freeze the excess for a future cook.";
    ...
    $("results").innerHTML=rows.length?rows.map(r=>{const specialBuy=purchaseDisplay(r.k,r.o,r.units,r.buyWeight),buy=r.buyOverride||(r.o.mode==="weight"?`BUY ${Math.ceil(r.bought*10)/10} lb of fillets`:r.k==="hog"?`BUY 1 whole hog (~${Math.ceil(r.buyWeight)} lb hanging weight)`:r.k==="brisket"&&choices[r.k]==="packer"?`BUY ${r.units} ${r.o.unit}${r.units>1?"s":""} (~${r.units*14}–${r.units*18} lb total)`:specialBuy||`BUY ${r.units} ${r.o.unit}${r.units>1?"s":""} (~${Math.ceil(r.buyWeight*10)/10} lb total)`),ex=r.k==="porkbelly"?"":(r.excess>.5?` • Planned excess: <span class="excess">${Math.round(r.excess*10)/10} lb</span>`:"");return `<div class="result"><div class="resultTop"><div><div class="resultTitle">${r.m.name}</div><span class="pill">${r.o.label}</span><span class="pill">${Math.round(r.y*100)}% yield</span></div><div class="buy">${buy}</div></div><div class="details">Finished meat needed: <b>${Math.round(r.finished*10)/10} lb</b> • Raw requirement: <b>${Math.round(r.bought*10)/10} lb</b>${r.o.mode==="units"&&r.k!=="hog"&&r.k!=="porkbelly"&&planningMode!=="family"?` • Planning unit: ${r.o.unitWeight} lb`:""}${ex}</div>${r.purchaseNote?`<div class="purchaseNote">${r.purchaseNote}</div>`:""}${r.k==="hog"?wholeHogWarning(r.buyWeight):""}</div>`}).join(""):"<p class='note'>Select at least one protein.</p>";calcSides();
  };
