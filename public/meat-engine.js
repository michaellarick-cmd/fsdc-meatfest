...
(() => {
  ...
  const PORTIONS = Object.freeze({
    brisketYield: .50, brisketFlatYield: .55, pmbeYield: .60, pbbeYield: .65,
    porkYield: .60, chickenYield: .62, chickenLegQuarterYield: .42,
    chickenThighYield: .52, fishYield: .76, primeRibYield: .80,
    turkeyYield: .55, turkeyBreastYield: .65, turkeyLegYield: .45,
    ribsYield: .70, bratYield: .90
  });
  ...
  function wholeHogPlan(finishedMeat, headFeet = 'on') {
    let onWeight = Math.max(.1, finishedMeat / .50);
    for (let i = 0; i < 40; i++) {
      const next = finishedMeat / wholeHogYield(onWeight);
      if (Math.abs(next - onWeight) < .0001) { onWeight = next; break; }
      onWeight = next;
    }
  ...
  function canonicalRow({ key, eaters, serving = STANDARD_SERVING, choice = {} }) {
    ...
    if (key === 'hog') {
      ...
      const ribsNeeded = eaters * ANCHORS.ribsTakeRate * ANCHORS.ribsPerTaker * (Number(serving) / STANDARD_SERVING);
      ...
      const linksNeeded = eaters * ANCHORS.bratsPerEater * (Number(serving) / STANDARD_SERVING);
      ...
      raw = birdsNeeded * ANCHORS.chickenLb;
  ...
  // Meatfest is an anchor-based planning model. When several proteins are
  // selected, each selected protein keeps its established Meatfest role and
  // scales with the headcount. We do NOT divide one finished-meat target
  // among selected proteins; that would change the established anchors.
  function multiProteinRows({ keys, eaters, serving = STANDARD_SERVING, choices = {} }) {
    if (!Array.isArray(keys) || !keys.length) return [];
    if (keys.includes('hog')) {
      return keys.length === 1
        ? [{ key: 'hog', row: canonicalRow({ key: 'hog', eaters, serving, choice: choices.hog || { headFeet: 'on' } }) }]
        : [];
    }
    return keys.map(key => {
      const choice = choices[key] || {};
      const row = canonicalRow({ key, eaters, serving, choice });
  ...
  globalThis.MeatEngine = Object.freeze({
    BASE_EATERS, STANDARD_SERVING, FAMILY_CUSHION, ANCHORS, PORTIONS,
    round1, roundUp, wholeHogYield, wholeHogPlan, canonicalRow,
    multiProteinRows, familyRow
  });
})();
