...
(function(){
  function tags(){
    ...
    (selected||[]).forEach(k=>{
      ...
      else if(k==="fish")t.add("fish");
      else if(k==="pork")t.add("pulled_pork");
      else if(k==="brisket")t.add("brisket");
      else if(k==="pmbe")t.add("pmbe");
      else if(k==="brats")t.add("brats");
      else if(k==="ribs")t.add("ribs");
      else if(k==="prime")t.add("prime_rib");
      else if(k==="hog")t.add("whole_hog");
      else if(k==="turkey")t.add("turkey");
      else if(k==="porkbelly")t.add("pork_belly");
    });
    return t;
  }
  function any(t,a){return a.some(x=>t.has(x));}
  window.sideRecommendation=function(id){
    const t=tags();
    switch(id){
  ...
  const refresh=()=>{if(typeof renderSideCards==="function")renderSideCards();};
  setTimeout(refresh,0); setTimeout(refresh,100); setTimeout(refresh,500);
})();
