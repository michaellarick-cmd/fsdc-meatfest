...
(()=>{
  ...
  const sync=()=>document.querySelectorAll('[data-side]').forEach(el=>{const id=el.dataset.side,on=selectedSides.has(id),rec=sideRecommendation(id);el.classList.toggle('on',on);el.classList.toggle('recommended',rec);el.setAttribute('aria-pressed',String(on));const badge=el.querySelector('[data-rec]');if(badge)badge.hidden=!rec});
