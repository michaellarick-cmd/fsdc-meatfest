...
test('station plan: every placed service item carries setup instructions',()=>{const layout=fullMenu().tables.layout;assert.equal(layout.stationPlan.length,4);const placed=layout.stationPlan.flatMap(t=>t.items);assert.ok(placed.length>0);for(const item of placed){assert.ok(item.name);assert.ok(item.station);assert.ok(item.vessel);assert.ok(item.service)}assert.ok(layout.stationPlan.some(t=>t.items.some(x=>x.name==='Sauerkraut'&&x.service.includes('spoon'))));assert.ok(layout.stationPlan.some(t=>t.items.some(x=>x.name==='Baked Beans'&&x.service.includes('ladle'))));assert.ok(layout.stationPlan.some(t=>t.items.some(x=>x.name==='Hawaiian Rolls'&&x.service.includes('bread'))))});
test('station plan: table records preserve guest-flow station order',()=>{const plan=fullMenu().tables.layout.stationPlan;const ranks=plan.flatMap(t=>t.stations.map(s=>['entry','cold','vegetable','starch','core','specialty','bread','finish'].indexOf(s.id)));assert.deepEqual(ranks,[...ranks].sort((a,b)=>a-b))});
