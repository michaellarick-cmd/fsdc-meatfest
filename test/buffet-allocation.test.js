...
test('physical allocation: cold sides stay on the left before starch/proteins',()=>{const t=fullMenu().tables.layout.segments,t1=itemNames(t[0]),t2=itemNames(t[1]);assert.ok(t1.includes('Cucumber Salad'));assert.ok(t1.includes('Coleslaw'));assert.ok(!t1.includes('Brisket'));assert.ok(t2.includes('Baked Beans'))});
test('physical allocation: sauerkraut stays with the protein-side zone',()=>{const t=fullMenu().tables.layout.segments[2];assert.ok(itemNames(t).includes('Sauerkraut'))});
