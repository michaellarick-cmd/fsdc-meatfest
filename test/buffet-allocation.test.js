...
test('physical allocation: full menu uses actual vessel widths including jars',()=>{const p=fullMenu();assert.ok(p.tables.linearRequired>264);assert.equal(p.tables.linearProvided,264);assert.equal(p.tables.overflow,true);assert.deepEqual(p.tables.layout.recommendedTables,[72,72,72,48,48])});
test('physical allocation: cold sides stay on the left before starch/proteins',()=>{const t=fullMenu().tables.layout.segments,t1=t[0].items.flatMap(g=>g.items).map(x=>x.name||x.id),t2=t[1].items.flatMap(g=>g.items).map(x=>x.name||x.id);assert.ok(t1.includes('Cucumber Salad'));assert.ok(t1.includes('Coleslaw'));assert.ok(!t1.includes('Brisket'));assert.ok(t2.includes('Baked Beans'))});
test('physical allocation: sauerkraut stays with the protein-side zone',()=>{const t=fullMenu().tables.layout.segments[2].items.flatMap(g=>g.items).map(x=>x.name||x.id);assert.ok(t.includes('Sauerkraut'))});
...
test('physical allocation: compact protein menu fits without false overflow',()=>{const p=B.plan({eaters:44,proteinKeys:['pork','brisket'],sideIds:['coleslaw','mac','beans'],breadIds:['hawaiian'],condimentIds:['bbqSauce'],dessertIds:[]});assert.equal(p.tables.overflow,false);assert.ok(p.tables.linearRequired<=264)});
