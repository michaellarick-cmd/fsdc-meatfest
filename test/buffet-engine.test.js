...
test('changing side selection does not create phantom sides',()=>{const p=B.sidePlan({sideIds:['beans'],proteinKeys:['brisket']});assert.deepEqual(p.map(x=>x.id),['beans']);});
...
test('functional buns are added only for selected supplemental meats',()=>{const p=B.breadPlan({supplementalIds:['burgers','hotdogs'],proteinKeys:[]});assert.deepEqual(p.map(x=>x.id).sort(),['burgerBuns','hotDogBuns']);});
