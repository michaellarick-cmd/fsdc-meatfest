...
(async()=>{
 ...
 try{
  ...
  for(let i=0;i<=40;i++){
   ...
   const target=Math.round(max*(i/40));
  ...
  const post=await page.evaluate(()=>({scrollY:scrollY,scrollHeight:document.documentElement.scrollHeight,rows:document.querySelectorAll('#buffetDynamic .b9row').length,tables:document.querySelectorAll('#buffetLayoutDynamic .b9table').length,controls:document.querySelectorAll('#buffetServiceCard button[data-k]').length}));
  if(post.scrollY<Math.max(0,post.scrollHeight-innerHeight-250))fail(`${BROWSER_NAME} scroll snapped away from bottom: ${JSON.stringify(post)}`);
