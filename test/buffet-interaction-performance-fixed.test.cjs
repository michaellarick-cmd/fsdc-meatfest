...
(async()=>{
 ...
 try{
  ...
  for(let i=0;i<=40;i++){
   ...
   await page.evaluate(target=>window.scrollTo(0,target),Math.round(maxScroll*(i/40)));
  ...
  for(const [kind,id,label] of selections){
   const button=page.locator(`meatfest-buffet button[data-kind="${kind}"][data-id="${id}"]`);
   ...
   const expectedTop=before.top-(after.scrollY-before.scrollY),topDelta=Math.abs(after.top-expectedTop);
   if(topDelta>100)fail(`${BROWSER_NAME}: ${label} moved unexpectedly: ${topDelta}px.`);
   const health=await page.evaluate(()=>{const root=document.querySelector('meatfest-buffet')?.shadowRoot;return{serviceHeight:root?.querySelector('#buffetServiceCard')?.getBoundingClientRect().height||0,layoutHeight:root?.querySelector('#buffetLayoutCard')?.getBoundingClientRect().height||0,scrollHeight:document.documentElement.scrollHeight,rows:root?.querySelectorAll('#buffetServiceCard .row').length||0,tables:root?.querySelectorAll('#buffetLayoutDynamic .table').length||0,controls:root?.querySelectorAll('#buffetServiceCard button[data-kind]').length||0}});
   if(health.serviceHeight<100||health.layoutHeight<100||health.scrollHeight<500||health.tables!==8||health.controls<10)fail(`${BROWSER_NAME}: ${label} left invalid layout: ${JSON.stringify(health)}`);
  ...
  console.log(`${BROWSER_NAME} mobile Buffet component interaction and scroll regression passed.`);
