...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>{const host=document.querySelector('meatfest-buffet');const root=host?.shadowRoot;const h=root?.querySelector('[data-mf-section="layout"] .table b');return !!h&&!/waiting for plan/.test(h.textContent||'')},{timeout:15000});
    ...
    const trace=[];
    for(let i=0;i<40;i++){await page.evaluate(()=>window.scrollBy(0,240));await page.waitForTimeout(25);trace.push(await page.evaluate(()=>({y:window.scrollY,h:document.documentElement.scrollHeight})))}
    ...
    const expectedMax=Math.max(0,maxHeight-VIEWPORT_HEIGHT),postsDuringScroll=await page.evaluate(()=>window.__meatfestWorkerPosts);
    ...
    console.log(`${BROWSER_NAME} mobile Buffet scroll isolation regression passed.`);console.log(JSON.stringify({browser:BROWSER_NAME,initialPosts,postsDuringScroll,trace,health},null,2));
