...
(async () => {
  ...
  try {
    ...
    if(state.executionRows!==state.setupItems||state.executionRows!==state.crewRows)fail(`Live buffet service artifacts disagree on item count: execution=${state.executionRows}, physical=${state.setupItems}, crew=${state.crewRows}`);
