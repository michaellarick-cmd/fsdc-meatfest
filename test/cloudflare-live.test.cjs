...
(async () => {
  ...
  try {
    ...
    const state=await page.evaluate(()=>{const summary=window.buildSummary(),B=window.BuffetEngine,rows=summary.sideRows||[],plan=B.sidePlan({sideIds:rows.map(r=>r.id),proteinKeys:summary.rows.map(r=>r.key),sideRows:rows}),cauli=rows.find(r=>r.id==='cauli'||r.id==='cauliflowerMac'),collards=rows.find(r=>r.id==='collards'),cp=plan.find(r=>r.id==='collards'),table=B.tableRequirement([{linearIn:102}],{tableLengths:[72,48]});window.populatePrint?.();const pb=document.querySelector('#psBuffet'),pts=pb?[...pb.querySelectorAll('.ps-bTable')]:[];return{summary,B,cauli,collards,cp,table,buffetText:document.querySelector('#buffetServiceCard')?.innerText||'',visual:!!document.querySelector('#buffetLayoutCard .visualLayout'),print:!!pb,printTables:pts.length,printText:pb?.innerText||''};});
    ...
    if(!state.collards||state.collards.q.amount<1.25||state.collards.q.unit!=='recipe')fail(`Live Collard Greens quantity is wrong: ${JSON.stringify(state.collards)}`);
    const expectedBowls=Math.max(1,Math.ceil(state.collards.q.amount-1e-9));
    if(!state.cp?.quantity?.service||state.cp.quantity.service.count!==expectedBowls||state.cp.quantity.service.label!=='serving bowl')fail(`Live Collard Greens service calculation is wrong: ${JSON.stringify(state.cp?.quantity?.service)}`);
