...
(async () => {
  ...
  try {
    ...
    const state=await page.evaluate(()=>{const summary=window.buildSummary(),B=window.BuffetEngine,rows=summary.sideRows||[],plan=B.sidePlan({sideIds:rows.map(r=>r.id),proteinKeys:summary.rows.map(r=>r.key),sideRows:rows}),cauli=rows.find(r=>r.id==='cauli'||r.id==='cauliflowerMac'),collards=rows.find(r=>r.id==='collards'),cp=plan.find(r=>r.id==='collards'),table=B.tableRequirement([{linearIn:102}],{tableLengths:[72,48]});window.populatePrint?.();const pb=document.querySelector('#psBuffet'),pts=pb?[...pb.querySelectorAll('.ps-bTable')]:[],execution=document.querySelector('#buffetLayoutCard .mf-execution');return{summary,B,cauli,collards,cp,table,buffetText:document.querySelector('#buffetServiceCard')?.innerText||'',visual:!!document.querySelector('#buffetLayoutCard .visualLayout'),executionText:execution?.innerText||'',executionVisible:!!execution&&!!execution.offsetParent,executionRows:execution?[...execution.querySelectorAll('.mf-exec-row:not(.mf-exec-head)')].length:0,print:!!pb,printTables:pts.length,printText:pb?.innerText||''};});
