...
(async () => {
  ...
  try {
    ...
    // Buffet planning is intentionally asynchronous: the main-thread selection must
    // remain responsive while the worker builds the complete physical/service plan.
    // Wait for that plan to materialize before asserting its downstream artifacts.
    await page.waitForFunction(()=>{
      const card=document.querySelector('#buffetLayoutCard');
      return !!card?.querySelector('.mf-visual') && !!card?.querySelector('.mf-execution') && !!card?.querySelector('.mf-physical') && !!card?.querySelector('.mf-flow');
    },{timeout:15000});
    const state=await page.evaluate(()=>{const summary=window.buildSummary(),B=window.BuffetEngine,rows=summary.sideRows||[],plan=B.sidePlan({sideIds:rows.map(r=>r.id),proteinKeys:summary.rows.map(r=>r.key),sideRows:rows}),cauli=rows.find(r=>r.id==='cauli'||r.id==='cauliflowerMac'),collards=rows.find(r=>r.id==='collards'),cp=plan.find(r=>r.id==='collards'),table=B.tableRequirement([{linearIn:102}],{tableLengths:[72,48]}),capacity=window.BuffetAllocation.allocate([{station:'core',linearIn:50,items:[{id:'protein-a',name:'Protein A',vessel:{type:'chafer'}}]},{station:'core',linearIn:50,items:[{id:'protein-b',name:'Protein B',vessel:{type:'chafer'}}]},{station:'specialty',linearIn:40,items:[{id:'specialty-a',name:'Specialty A',vessel:{type:'chafer'}}]}],[72,48]);window.populatePrint?.();const execution=document.querySelector('#buffetLayoutCard .mf-execution'),presentationText=execution?.innerText||'',pb=document.querySelector('#psBuffet'),pts=pb?[...pb.querySelectorAll('.ps-bTable')]:[],sequenceText=execution?.querySelector('.mf-service-sequence')?.innerText||'',sequenceRows=execution?[...execution.querySelectorAll('.mf-service-sequence .mf-position-item')]:[],crew= document.querySelector('#psBuffet .ps-bAllocation'),crewText=crew?.innerText||'',crewRows=crew?[...crew.querySelectorAll('.ps-bCrewTable .ps-bCrewRow:not(.ps-bCrewHeadRow)')]:[],crewTables=crew?[...crew.querySelectorAll('.ps-bCrewTable')]:[],crewSetupRows=crewTables[0]?[...crewTables[0].querySelectorAll('.ps-bCrewRow:not(.ps-bCrewHeadRow)')]:[],crewSequence=crew?.querySelectorAll('.ps-bSequence li').length||0,physical=document.querySelector('#buffetLayoutCard .mf-physical'),physicalText=physical?.innerText||'',setupTables=physical?[...physical.querySelectorAll('.mf-setup-table')]:[],setupItems=physical?[...physical.querySelectorAll('.mf-setup-table .mf-position-item')]:[],map=document.querySelector('#buffetLayoutCard .mf-visual'),mapText=map?.innerText||'',flow=document.querySelector('#buffetLayoutCard .mf-flow:has(.mf-flow-route)'),flowText=flow?.innerText||'',execRows=execution?[...execution.querySelectorAll('.mf-exec-row:not(.mf-exec-head)')]:[],positionText=setupItems.map(x=>x.innerText).join(' | '),crewSetupText=crewSetupRows.map(x=>x.innerText).join(' | ');return{summary,B,cauli,collards,cp,table,capacity,buffetText:document.querySelector('#buffetServiceCard')?.innerText||'',visual:!!map,executionText:execution?.innerText||'',executionVisible:!!execution&&!!execution.offsetParent,executionRows:execRows.length,executionItemNames:execRows.map(x=>x.children[0]?.innerText||''),presentationText,sequenceText,sequenceRows:sequenceRows.length,print:!!pb,printTables:pts.length,printText:pb?.innerText||'',crew:!!crew,crewText,crewRows:crewRows.length,crewSetupRows:crewSetupRows.length,crewSequence,physical:!!physical,physicalText,setupTables:setupTables.length,setupItems:setupItems.length,mapText,flow:!!flow,flowText,physicalItemText:positionText,crewSetupText};});
