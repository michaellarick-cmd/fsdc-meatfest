...
test('48 adult-equivalent six-protein Meatfest applies the 60% floor to the overall portion only',()=>{const ids=['chicken','pork','brats','brisket','pmbe','ribs'];const rows=calculateFinishedProtein({adults:48,kids:0,selected:ids,serving:1/3});const by=Object.fromEntries(rows.map(r=>[r.id,r.finishedLb]));assert.ok(Math.abs(by.pmbe-9.6)<1e-6);assert.ok(Math.abs(by.brats-9.6)<1e-6);assert.ok(Math.abs(by.ribs-9.6)<1e-6);assert.ok(rows.every(r=>r.takeRate===.60));});
test('raw yield and purchase-unit rounding remain deterministic',()=>{assert.ok(Math.abs(calculateRawRequirement({finishedLb:9.6,yieldRate:.6})-16)<1e-6);const p=roundUpPurchase(9.6,4);assert.equal(p.units,3);assert.equal(p.purchasedLb,12)});
