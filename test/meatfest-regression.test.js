...
test('canonical purchase assumptions remain locked',()=>{
  assert.equal(A.bratLinkLb,.5); assert.equal(A.chickenLb,5); assert.equal(A.pmbeRoastLb,4);
  assert.equal(A.ribsRackLb,2.25); assert.equal(A.pulledPorkUnitLb,8.5); assert.equal(A.brisketUnitLb,14);
});

test('protein-count no longer silently multiplies or reduces Meatfest portions',()=>{assert.equal(multiplier(1),1);assert.equal(multiplier(6),1)});
...
test('48 adult-equivalent standard Meatfest anchors produce the validated finished targets',()=>{
  ...
  assert.equal(byId.brisket,9.75); assert.equal(byId.pmbe,9.6); assert.equal(byId.pork,10.2);
  assert.equal(byId.chicken,12.4); assert.ok(Math.abs(byId.ribs-7.875)<1e-12); assert.equal(byId.brats,3.6);
});

test('portion selector scales the Meatfest anchors',()=>{
  const standard=calculateFinishedProtein({adults:48,kids:0,selected:['chicken'],serving:1/3});
  const light=calculateFinishedProtein({adults:48,kids:0,selected:['chicken'],serving:1/4});
  const generous=calculateFinishedProtein({adults:48,kids:0,selected:['chicken'],serving:1/2});
...
test('raw yield and purchase-unit rounding are deterministic',()=>{
  ...
  const p=roundUpPurchase(2.6666666667,4); assert.equal(p.units,1); assert.equal(p.purchasedLb,4);
});
