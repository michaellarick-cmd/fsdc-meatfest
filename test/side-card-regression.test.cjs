...
(async()=>{
  ...
  try{
    ...
    for(const [id,label] of [['rolls','Hawaiian Rolls'],['cornbread','Cornbread']]){
      ...
      const before=await card.evaluate(el=>({on:el.classList.contains('on'),onclick:typeof el.onclick,bound:!!el._meatfestSideBound,listener:el._meatfestSideBound||false,html:el.outerHTML.slice(0,600)}));
      console.log(`${BROWSER_NAME} ${label} before tap ${JSON.stringify(before)}`);
      await card.tap();
      await page.waitForTimeout(250);
      const after=await card.evaluate(el=>({on:el.classList.contains('on'),onclick:typeof el.onclick,bound:!!el._meatfestSideBound,html:el.outerHTML.slice(0,600)}));
      console.log(`${BROWSER_NAME} ${label} after tap ${JSON.stringify(after)}`);
